  ...
  <v-dialog v-model="modalHapus" max-width="50%">
    <v-card>
      <v-toolbar dark color="primary" dense flat>
        <v-toolbar-title class="subtitle-1">Hapus Data</v-toolbar-title>
        <v-spacer></v-spacer>
...
export default {
  ...
  methods: {
    async hapus() {
      ...
      const url = process.env.VUE_APP_API_BASE + 'data/' + this.hapusItem.data_id
      this.http
        .delete(url, {
          headers: {
